...
async function killProcess(pid: number) {
  return new Promise<void>((resolve) => {
    ps.kill(pid, { signal: 'SIGTERM' }, () => resolve());
  });
}
...
async function waitForServerReady(url: string, timeout = 60000): Promise<boolean> {
  ...
  console.log('Server readiness check timed out after', timeout, 'ms');
  return false;
}
...
async function startViteServer() {
  return new Promise<void>((resolve, reject) => {
    try {
      console.log('Starting new Vite process...');
...
async function restartDevServer() {
  try {
    console.log('Attempting to stop existing Vite process...');
    
    // Kill existing process if we have one
    if (viteProcess) {
      console.log('Killing tracked Vite process');
      viteProcess.kill();
      viteProcess = null;
    }
    ...
    if (pid) {
      console.log(`Found additional Vite process (PID: ${pid}), stopping it...`);
    ...
    console.log('Dev server restart completed successfully');
  } catch (error) {
...
export async function waitForServer(url: string, maxAttempts = 3): Promise<boolean> {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      console.log(`Checking server (attempt ${attempt + 1}/${maxAttempts}) at ${currentUrl}...`);
      ...
      console.log('Server is responding normally with expected content');
      return true;
    } catch (error) {
