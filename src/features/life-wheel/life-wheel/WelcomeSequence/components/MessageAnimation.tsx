...
const MessageAnimation = ({ message, onComplete, userName = '' }: MessageAnimationProps) => {
  ...
  const getTextWithHighlights = (text: string, highlightWords: string[] = []) => {
    ...
    return (
      <>
        {parts.map((part, idx) => 
          part.highlight ? 
            <span key={idx} className={styles.highlight}>{part.text}</span> : 
